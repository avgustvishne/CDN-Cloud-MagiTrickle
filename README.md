...
Для основных профилей автоматически рассчитываются:

- **точное покрытие адресного пространства** IPv4/IPv6;
- **плотность покрытия** — адресов на один CIDR;
- **новое покрытие** при переходе `MINIMAL → PERFORMANCE → BALANCED → FULL`;
- **перекрытие** между соседними профилями;
- контроль, что каждый следующий профиль не теряет покрытие предыдущего;
- защита от аномального резкого изменения количества CIDR или покрытия.

Машиночитаемый отчёт: [**profile-intelligence.json**](data/profile-intelligence.json).
...
Сейчас контролируемое расширение такое:

- **PERFORMANCE:** `Hetzner` + `OVH`;
- **BALANCED:** `Melbicom` + `BuyVM` + `Contabo`;
- **MINIMAL:** не расширяется;
- **FULL:** сохраняет все настроенные провайдеры.

В профиль попадает только тот IPv4-префикс кандидата, для которого `dpi-ch` дал одновременно:

- `Alive = OK` или `OK_CUSTOM_HTTP`;
- `TCP 16-20 = OK`;
- `Siberian = OK`;
- все проверенные образцы внутри этого префикса прошли проверки.

GitHub Actions **не запускает DPI-проверку вместо пользователя**: runner находится в другой сети и не является доказательством поведения вашего ISP. `dpi-ch` сохраняет JSON-отчёт, который затем импортируется в `data/dpi-intelligence.json`. Свежесть evidence ограничена 72 часами; IPv6 пока не продвигается этим механизмом, поскольку текущий `dpi-ch`-контур проверяет IPv4.

Исходный JSON-отчёт `dpi-ch` **не кладётся в публичный репозиторий**. Локальный `scripts/import_dpi_results.py` превращает его в обезличенный `data/dpi-intelligence.json`; именно этот файл можно публиковать. Обычный update workflow затем использует его для перестройки профилей. Если свежего evidence нет, кандидаты **не добавляются** — существующие профили продолжают работать без расширения.

Это сознательно сделано консервативно: проверка `dpi-ch` является выборкой хостов внутри префикса, поэтому прохождение теста не трактуется как гарантия каждого IP в диапазоне.
## 🔄 Обновление

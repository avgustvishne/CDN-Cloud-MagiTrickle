...
Основной генератор — V14.\n\nV14 добавляет DPI-aware режим: после проверки Hyperion TCP 16–20 можно отметить провайдеры как `pass`, `fail` или `unknown` в `config/dpi-status.json`. Генератор автоматически собирает отдельные `dpi-recommended-v4.txt` и `dpi-recommended-v6.txt` только из провайдеров со статусом `pass`. `unknown` не считается безопасным статусом.

V12 добавляет дополнительные cloud/hosting-провайдеры и усиливает контроль обновлений IPv4/IPv6, дубликатов ASN, аномальных объёмов и целостности файлов.
